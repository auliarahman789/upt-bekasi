...
const TowerKritisPage: React.FC = () => {
  ...
  const getTowerChartData = (): ChartData[] => {
    ...
    // Apply the same filters used for the table
    const filteredData = apiData.data.filter((item) => {
      ...
      if (filterProgress !== "all") {
        ...
        if (filterProgress === "100") matchesProgress = progressValue === 100;
        else if (filterProgress === "0") matchesProgress = progressValue === 0;
    ...
    // Group filtered data by ULTG
    const ultgGroups: Record<string, TowerData[]> = {};
    filteredData.forEach((item) => {
      if (!ultgGroups[item.ultg]) ultgGroups[item.ultg] = [];
  ...
  return (
    <DefaultLayout>
      <div className="p-4 md:p-8 bg-gray-50 min-h-screen">
        ...
        {/* 🔹 Chart section (auto updates on filter change) */}
            ...
            currentItems.map((item, mobileIndex) => (
              ...
              >
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    <span className="font-medium text-gray-600">No.Tower:</span>
