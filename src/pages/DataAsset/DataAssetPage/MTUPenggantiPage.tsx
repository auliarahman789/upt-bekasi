...
import UsulanPenggantiTab from "./UsulanPengganti";
...
interface MTUDataItem {
  ...
  ultg: string;
  usulan_relokasi_bay: string;
  usulan_relokasi_gi: string;
}
...
const MTUPenggantiPage = () => {
  ...
  const [apiUsulanData, setApiUsulanData] = useState<UsulanPenggantiDataItem[]>(
    []
  );
  const [activeTab, setActiveTab] = useState<"penggantian" | "usulan">(
    "penggantian"
  );
  ...
  const transformedTableData = useMemo((): TransformedTableItem[] => {
    return apiData?.map((item, index) => ({
      ...
      usulan_relokasi_bay: item.usulan_relokasi_bay,
      usulan_relokasi_gi: item.usulan_relokasi_gi,
    }));
  }, [apiData]);
  ...
  const fetchMTUUsulanPenggantian = async (): Promise<void> => {
    try {
      setIsLoading(true);
      const response = await axios.get(
        `${
          import.meta.env.VITE_API_LINK_BE
        }/api/data-asset/mtu/usulan-penggantian`
      );

      if (response.data.status === "success") {
        setApiUsulanData(response.data.data?.data || []);
      }
    } catch (err) {
      console.error("Error fetching MTU usulan penggantian data:", err);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="p-3 md:p-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 md:mb-6 gap-4">
            ...
            {/* Only show location filters when penggantian tab is active */}
            {activeTab === "penggantian" && (
              <>
                ...
                <div className="sm:hidden w-full">
                  ...
                  >
                    <span className="text-sm font-medium text-gray-900">
                      {locations.find((loc) => loc.key === activeLocation)
          ...
          {activeTab === "penggantian" ? (
            ...
            <UsulanPenggantiTab data={apiUsulanData} />
          )}
        </div>
      </div>
    </DefaultLayout>
